...
if (isset($grado) && isset($periodo)  && isset($sede)) {
  ...
  $sqls="SELECT nombre from sedes where idsede='$sede'";
    ...
    $pdf->Ln();
     $pdf->SetX(90);
    $pdf->Cell(40, 5, 'MATEMATICAS', 1, 0, 'C', 1);
    $pdf->SetX(130);
...
//Matematicas 
    ...
    //Lenguaje
    $sqlp3 = "SELECT distinct  a.acronimo, a.idasignatura from asignaturas a inner join calificaciones c on a.idasignatura=c.idasignatura where c.idgrado='$grado' and c.periodo='$periodo' and orden>=2.1 and orden<=2.2 order by orden asc ";
        ...
        $pdf->Cell(10, 5, 'DEF',  1, 0, 'J');     
    //Ciencias naturales
    $sqlp4 = "SELECT distinct  a.acronimo, a.idasignatura from asignaturas a inner join calificaciones c on a.idasignatura=c.idasignatura where c.idgrado='$grado' and c.periodo='$periodo' and orden>4 and orden<=4.3 order by orden asc ";
        ...
        $pdf->Cell(10, 5, 'DEF',  1, 0, 'J'); 
    //SOCIALES
    $sqlp5 = "SELECT distinct  a.acronimo, a.idasignatura from asignaturas a inner join calificaciones c on a.idasignatura=c.idasignatura where c.idgrado='$grado' and c.periodo='$periodo' and orden>5 and orden<=5.4 order by orden asc ";
        ...
        $pdf->Cell(10, 5, 'DEF',  1, 0, 'J');    
     //Otras Materias
    $sqlp7 = "SELECT distinct  a.acronimo, a.idasignatura from asignaturas a inner join calificaciones c on a.idasignatura=c.idasignatura where c.idgrado='$grado' and c.periodo='$periodo' and orden>=5.9 order by orden asc ";
    ...
    $notaNl=0;
      ...
      $por=($reg_cg[0]/100);              
            ...
            $pdf->Cell(10, 5, round($mat, 1),  1, 0, 'J', 1);  
            if($mat<3){
              $cp=$cp+1;
            } else {
              $cg=$cg+1;
...
$sqlp3 = "SELECT nota, idasignatura from calificaciones where idestudiante='$idest'  and  idgrado='$grado' and idsede='$sede' and periodo ='$periodo' and orden>=2.1 and orden<=2.2  order by orden asc";
      ...
      $por=($reg_cg[0]/100);              
            ...
            $pdf->Cell(10, 5, round($len, 1),  1, 0, 'J', 1);
            if($len<3){
             $cp=$cp+1;
            } else {
              $cg=$cg+1;
...
//Ciencias Naturales
$sqlp4 = "SELECT nota, idasignatura from calificaciones where idestudiante='$idest'  and  idgrado='$grado' and idsede='$sede' and periodo ='$periodo' and orden>=4.1 and orden<=4.3  order by orden asc";
            $resp4 = $mysqli->query($sqlp4);
$numn4=$resp4->num_rows;
  if ($numn4>0) {  
      ...
      $por=($reg_cg[0]/100);              
            ...
            $pdf->Cell(10, 5, round($cn,1),  1, 0, 'J', 1);
            if($cn<3){
              $cp=$cp+1;
            } else {
             $cg=$cg+1;
...
//sociales
$sqlp5 = "SELECT nota, idasignatura from calificaciones where idestudiante='$idest'  and  idgrado='$grado' and idsede='$sede' and periodo ='$periodo' and orden>=5.1 and orden<=5.4  order by orden asc";
            $resp5 = $mysqli->query($sqlp5);
            while ( $regp5 = $resp5->fetch_row()) {
              $ida=$regp5['1'];
      $sql_cg="SELECT porcentaje from carga_academica where idasignatura='$ida' and idgrado='$grado'";
      ...
      $por=($reg_cg[0]/100);              
            ...
            $pdf->Cell(10, 5, round($fil, 1),  1, 0, 'J', 1);
            if($fil<3){
             $cp=$cp+1;
            } else {
              $cg=$cg+1;
...
//Otras Materias
            $numn1=0;
$sqlp7 = "SELECT nota, idasignatura from calificaciones where idestudiante='$idest'  and  idgrado='$grado' and idsede='$sede' and periodo ='$periodo' and orden>=5.9   order by orden asc";
            $resp7 = $mysqli->query($sqlp7); 
  ...
  if ($numn1>0) {                       
      ...
      $acmat=($mat+$len+$cn+$fil+$acom);
      $conta=($c1+$c2+$c3+$c4+$c6);
      if($acmat>0 && $conta>0){
        $prom=$acmat/$conta;
        $rprom=round($prom,1);
      }
