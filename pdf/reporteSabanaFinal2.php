...
//CUARTO PERIODO
$sqlp2 = "SELECT nota, idasignatura from calificaciones where idestudiante='$idest'  and  idgrado='$grado' and idsede='$sede' and periodo ='4' and orden>1 and orden<2  order by orden asc";
            $resp2 = $mysqli->query($sqlp2);
            while ( $regp2 = $resp2->fetch_row()) {
              $ida=$regp2['1'];
      $sql_cg="SELECT porcentaje from carga_academica where idasignatura='$ida' and idgrado='$grado'";
      ...
      $por=($reg_cg[0]/100);           
            ...
            $pmat=round((($mat+$mat2+$mat3+$mat4)/4),1);
            if($pmat<3){
              $cp=$cp+1;
            ...
            $pdf->Cell(5, 5, round($mat,1), 1,0 , 'J');
            $pdf->Cell(5, 5, round($mat2,1), 1,0 , 'J');
            $pdf->Cell(5, 5, round($mat3,1), 1,0 , 'J');
            $pdf->Cell(5, 5, round($mat4,1), 1,0 , 'J');
            $pdf->SetFont('Arial', 'B', 6.5);           
            if($notaNF>0){
