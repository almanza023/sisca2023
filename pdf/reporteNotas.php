...
$pdf->Ln(11);    
$pdf->Cell(200, 6, utf8_decode('LISTADO DE NOTAS ACADÉMICAS'), 1, 0, 'C', 1); 
$pdf->Ln(8);    
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(10, 6, utf8_decode('N°'), 1, 0, 'C', 1);
$pdf->Cell(130, 6, utf8_decode('ESTUDIANTES'), 1, 0, 'C', 1);
$pdf->Cell(30, 6, utf8_decode('NOTA'), 1, 0, 'C', 1);
$pdf->Cell(30, 6, utf8_decode('LOG COG'), 1, 0, 'C', 1);    
$pdf->Ln();
$sql = "SELECT e.apellidos, e.nombres, c.nota, c.logro_c from calificaciones c INNER JOIN estudiantes e ON c.idestudiante=e.idestudiante where c.idgrado='$grado' and c.idasignatura='$asignatura' and c.periodo='$periodo' and c.idsede='$sede' order by e.apellidos";
    ...
    while ($reg = $res->fetch_row()) {
        ...
        $pdf->Cell(130, 6, utf8_decode($reg['0'].' '.$reg['1']), 1, 0, 'J');
        $pdf->Cell(30, 6, $nota, 1, 0, 'J');
        $pdf->MultiCell(30, '6',$reg[3], 1, 1,1);
        $ac=$ac+$nota;  
