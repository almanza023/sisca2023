<?php 
/*FORMATO BOLETINES PARA LOS GRADOS
3, 4, 5 */
$sede   = $_POST['sede'];
$grado   = $_POST['grado'];
...
while ($i <= $num1) {
  while ($reg = $res->fetch_row()) {
    ...
    $pdf->SetFont('Arial', 'B', 10);          
    $pdf->Ln(2);
    $pdf->Cell(198, 6, utf8_decode(' INFORME ACADÉMICO Y/O DISCIPLINARIO'), 1, 1, 'C', 1); $pdf->SetFont('Arial', '', 8);
    $idest= $reg['0'];  
    $nom = $reg['1'] . ' ' . $reg['2'];
    $pdf->Cell(113, 4, 'Nombres: ' . utf8_decode($nom), 1, 0, 'J');
    $pdf->Cell(47, 4, 'Grado: ' . $reg['3'], 1, 0, 'J');
...
$pdf->Cell(190, 3,utf8_decode($nom_ac) , 0, 1, 'J');            
$pdf->Cell(10, 1, ' Director de Grupo', 0, 1, 'J');
$pdf->Ln();        
    ...
    while ($reg1 = $res1->fetch_row()) {                
      ...
      while ($regm=$resm->fetch_row()) {
        ...
        if ($nota<3 && !empty($notaN)) {
          $por=$regm['1']/100;            
          $nt=round($notaN*$por,1);                                              
          $def=($nper1+$nper2+$notaN)/3;  
          $ac=$ac+$nt;
        } else {
          $por=$regm['1']/100;            
          $nt=round($nota*$por,1);                                              
          $def=($nper1+$nper2+$nota)/3;  
          $ac=$ac+$nt;
        }            
        $pdf->SetX(144);
        $pdf->Cell(16, 6, $reg1['5'], 1, 0, 'J');
